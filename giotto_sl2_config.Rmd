...
# Fig. 2D
## Azimuth Human PBMC atlas (Hao et al., Cell 2021), which is based on CITEseq data was used to include comprehensive gene sets #for cell types in Figure 2D and text, where applicable (lines 97, 147-150, 153-4, 190-92). The following markers more #definitively identify T cell subsets as:
#CD4+ naive T cells (CD3D/G+, CD4+, CCR7+, IL7Rlow)
#CD4+ memory T cells (CD3D/G+, CD4+, IL7Rhigh, CCR7neg, KLRB1+)
#CD8+ naive T cells (CD3D/E+, CD8A/B+, IL7R+, SIGIRR+, CCR7+)
#CD8+ memory T cells (CD3D/E+, CD8A/B+, CST7+, NKG7+, GZMH+, GZMA+, GZMK+, CCL5+)
#Tregs (CD3D/G+, FOXP3+, CD4+, IL2RA+, TIGIT+, CTLA4+)
#these additional markers were then included in the heatmap in Fig.2D
# Gini
...
# Create a vector of your additional markers
additional_markers <- unique(c("IL7R","LTB","CD4","CD3D","CD3G","CD4","CCR7","IL7R","NOSIP","IL7R","CD4","ITGB1","LTB","AQP3","IL32","IL7R","CCL5","FYB1","GZMK","IL32","GZMA","KLRB1","LTB","AQP3","CD8B","CD8A","CD3D","HCST","CD3G","LINC02446","CTSW","CD3E","CD8B","S100B","CCR7", "NOSIP","LINC02446", "CD8A", "CD8B", "ANXA1","CD8A", "KRT1", "LINC02446" ,"YBX3", "IL7R","CCL5", "GZMH", "CD8A", "NKG7", "GZMK", "CST7", "CD8B","FOXP3", "CD4",   "IL2RA" ,"TIGIT", "CTLA4", "CD79A",    "MS4A1",    "CD74",     "HLA-DQA1", "IGHM"))  # add your desired markers

# Combine with topgini_genes
all_markers <- unique(c(topgini_genes, additional_markers))

# Use the combined markers in the plot
# Create combined feature order
combined_feat_order <- unique(c(unique(markers$feats), additional_markers))

# Use the combined markers in the plot
plotMetaDataHeatmap(gem_nomix, 
                    selected_feats = all_markers,
                    custom_feat_order = combined_feat_order,  # Updated this line
                    ...
                    return_plot = TRUE,
                    show_plot = TRUE,
                    save_param = list(save_folder = 'niche_plots/',
                           save_name = 'heatmap_gini_additional_markers_modified_Fig2D',  # Updated name
                           save_format = 'pdf',
                           units = 'in'))


# plot patient wise cell type cell number data Supplemental Figure 3B 
cell_metadata<- pDataDT(gem)
...
```{r visualise gem}
#fig 2c
#in figure 2c, mac rich is replaced by mac2 and igg rich is replaced by plasmablast2, fib rich by fib2
